...
class LanguageController {
    ...
    public static async getOneById(req: Request, res: Response): Promise<void> {
        ...
        try {
            language = await languageRepo.findOneOrFail({ where: [{ id: languageId }] })
        } catch (error) {
            console.error(error);
    ...
    public static async createLanguage(req: Request, res: Response): Promise<void> {
        const languageRepo: Repository<Language> = getRepository(Language);
        const { name } = req.body
        ...
        try {
            ...
            new APIResponse(res, 500, {}, ['Error while saving language to the database.', error]).send();
            return;
        }
    ...
    public static async deleteLanguage(req: Request, res: Response): Promise<void> {
        const languageId: number = Number(req.params.id)
        const languageRepo: Repository<Language> = getRepository(Language);
        ...
        try {
            ...
            new APIResponse(res, 404, {}, ["Couldn't find a language with given id."]).send()
            return;
        }
